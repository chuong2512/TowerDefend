...
namespace MadLevelManager
{
	public class MadiTween : MonoBehaviour
	{
		...
		{
			Component[] components = target.GetComponents<MadiTween>();
			Component[] array = components;
			for (int i = 0; i < array.Length; i++)
			{
				MadiTween madiTween = (MadiTween)array[i];
				madiTween.enabled = true;
			}
		}
		...
		public static void Resume(GameObject target, string type)
		{
			...
			{
				MadiTween madiTween = (MadiTween)array[i];
				string text = madiTween.type + madiTween.method;
		...
		public static void Pause(GameObject target)
		{
			...
			{
				MadiTween madiTween = (MadiTween)array[i];
				if (madiTween.delay > 0f)
				{
					madiTween.delay -= Time.time - madiTween.delayStarted;
					madiTween.StopCoroutine("TweenDelay");
				}
				madiTween.isPaused = true;
				madiTween.enabled = false;
			}
		}
		...
		{
			Component[] components = GetComponents<MadiTween>();
			Component[] array = components;
			int num = 0;
			MadiTween madiTween;
			while (true)
			{
				...
				madiTween = (MadiTween)array[num];
				if (madiTween.type == "value")
				{
					return;
				}
				if (madiTween.isRunning && madiTween.type == type)
				{
					if (madiTween.method != method)
					{
						return;
					}
					if (madiTween.tweenArguments.Count != tweenArguments.Count)
					{
						break;
					}
					...
					try
					{
						...
						{
							DictionaryEntry dictionaryEntry = (DictionaryEntry)enumerator.Current;
							if (!madiTween.tweenArguments.Contains(dictionaryEntry.Key))
							{
								madiTween.Dispose();
								return;
							}
							if (!madiTween.tweenArguments[dictionaryEntry.Key].Equals(tweenArguments[dictionaryEntry.Key]) && (string)dictionaryEntry.Key != "id")
							{
								madiTween.Dispose();
								return;
							}
